...
### ③ 채권 디코더는 실회선 전문을 한 건도 못 읽는다 — 미해결
...
`00009987.500` 처럼 소수점 있는 가격인데 디코더는 정수를 기다린다. 합성 전문 빌더도 같은 오해로
만들어져 테스트는 통과한다 — §4 의 두 번째 사례. 채권은 지금 conf 에 없어 이번엔 안 고쳤다.
고칠 때는 `layouts.md` 의 채권 인터페이스(IFMSRPD0028~0031)를 이 캡처와 대조하고 빌더부터 고친다.
