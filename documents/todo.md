...
10단은 주식선물(`04F`)·주식옵션(`05F`)뿐이라 `depth_for(trcode)` 가 상품군으로 고른다.
...
### ② 상품군 `17F` 는 표준서 v1.341 에 없다

| 코드 | 건수 | 표준서 |
|---|--:|---|
| `B617F` | 6,925 | 인터페이스목록에 없음 |
| `V117F` | 180 | 없음 |
| `G717F` | 31 | 없음 |

별첨-정보구분코드도 `16F`(코스피200 위클리옵션)까지고 `17F` 항목이 없다.
**정체 미확인** — 표준서 이후 추가된 상품군으로 보이나 확인 전에는 적지 않는다.

> ⚠️ **기동 검증을 "표에 없으면 실패" 로 두면 안 된다.** §5 에 그렇게 적어 뒀는데,
> 표준서가 실제 회선보다 뒤처진다는 게 확인됐다. 없는 코드는 **경고 + 명시적 허용**으로
> 간다. 다행히 파생 디코더는 표가 없어도 돈다 — 길이는 `depth × 46 + 고정` 이고
> `depth_for()` 가 `04F`/`05F` 만 10단으로 고르므로 `17F` 는 자동으로 5단 324B 다.
