...
- 코어 예산: 8물리/16논리(Ryzen 7 7700)에서 핀할 스레드가 feed 2 + 전략 2 = **4물리**
  (SMT 형제 포함 8논리). 라우터가 1물리를 더 먹으면 OS 가 2물리에서 돈다.
...
- **M=2 에서는 피드당 SPSC 링 2개로 시작한다.** 멀티리더 브로드캐스트 링은
  덮어쓰기·랩어라운드 처리가 붙고, SPSC 는 소비자별 백프레셔가 분리된다.
...
- [ ] shm 링 구현 (Windows named mapping, SPSC ×2)
- [ ] 피드 사망 시 청산 허용 여부 (§9)
