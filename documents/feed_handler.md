...
소비자 쪽도 이대로 짰다 (fractal-engine 2026-09-14): `Broker` 스레드를 지우고 각 `TradingEngine` 이
`engine/feed.rs` 의 `FeedInput` 으로 세그먼트마다 자기 `RingConsumer` 를 붙여 읽는다. 자기 종목이 아닌
레코드는 헤더 심볼만 alias 로 풀어 보고 디코드 전에 버리고, 나머지는 엔진 소유 버퍼에 적응해 빌려 쓴다.
`Lagged(n)`·재시작은 델타 FSM 이 있는 종목 전부에 복구 트리거.
...
~~`RECOVERED` 는 델타 피드를 붙일 때 추가한다. 지금은 불필요.~~ → **영구히 넣지 않는다** (2026-09-14). 델타 복구는 소비자(fractal-engine) 안에서 시작하고 끝나므로 와이어가 표시할 사건이 없다 (`todo.md` §17⑥).
...
- [ ] 복구 경계 후속 — OKX·Bybit REST 스냅샷 본문 디코더, REST `SEQUENCE` 카운터가 델타 ID 와 같은지 거래소별 확인 (`todo.md` §17④)
