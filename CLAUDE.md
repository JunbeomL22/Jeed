...
| 상품 | 형식 | 소수 |
|---|---|---|
| 주식선물·주식옵션·상품파생 | `[부호][정수 8]` | 0 |
| 코스피200 선·옵, 코스닥150, 국채·금리·통화 파생 | `[부호][정수 5][.][소수 2]` | 2 |
| 3개월무위험지표금리선물 | `[부호][정수 4][.][소수 3]` | 3 |

**상품군(trcode 뒤 3자리)으로도 안 갈린다** — 3개월무위험지표금리선물은 국채/금리파생과
같은 `06F` 인데 형식이 다르다. 그래서 **전문 안에서 `.` 위치를 찾는다.** 9바이트 한 번
훑으면 값과 소수자리수가 같이 나오고, 종목별 스케일 표를 들고 다닐 필요가 없다.
없으면 소수 0. 이 값이 레코드 헤더의 `price_scale` 이 된다.

부호는 양수 `'0'`, 음수 `'-'` (스프레드 호가에만 음수가 온다).
증권은 11바이트 `[부호][미사용][유효숫자 9]` 인데, 미사용 자리가 `'0'` 이라 같은 파서로 읽힌다.
