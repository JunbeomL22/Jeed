...
//!
//! The same double `jeed_krx::tests::recv::sink` uses, and for the same reason.
//! A `Vec` that only ever saw finished records would hide the one bug the real
//! slot is shaped to prevent: an adapter that writes half a record and fails.
//! So the buffer is **reused** exactly as a ring slot is, and a failed
...
fn a_failed_fill_publishes_nothing() {
    ...
    assert_eq!(out, Err("adapter gave up"));
