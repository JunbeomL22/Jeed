// Allow redundant slicing in tests
...
fn test_to_u128_decimal() {
    ...
    for i in 1..U128_LENGTH_BOUND {
        ...
        assert_eq!(
            val,
            std::str::from_utf8(x).unwrap().parse::<u128>().unwrap(),
            "Failed for {} bytes",
            i
        );
    }
...
fn test_leading_zero_u128() {
    ...
    assert_eq!(val_leading_zeros, 123_456_789_012_345_678_901_234_000_000);
}

#[test]
fn test_back_and_forth() {
    for i in 1_u128..1_000_000 {
        let x = i.to_string();
...
fn test_base() {
    ...
    for test in test_vecs.iter() {
        let x = test.as_bytes();
        let val = u128::parse_decimal(x).unwrap();
        assert_eq!(val, test.parse::<u128>().unwrap());
    }
}
